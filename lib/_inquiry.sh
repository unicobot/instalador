...
get_link_git() {
  ...
  print_banner
  printf "${WHITE} 💻 Insira o link do GITHUB do seu UnicoBot que deseja instalar:${GRAY_LIGHT}"
  printf "\n\n"
  read -p "> " link_git
}
...
inquiry_options() {
  ...
  print_banner
  printf "${WHITE} 💻 Bem vindo(a) ao Gerenciador UnicoBot, selecione abaixo a proxima ação!${GRAY_LIGHT}"
  printf "\n\n"
  printf "   [0] Instalar UnicoBot\n"
  printf "   [1] Atualizar UnicoBotP\n"
  printf "   [2] Deletar UnicoBot\n"
  printf "   [3] Bloquear UnicoBotP\n"
  printf "   [4] Desbloquear UnicoBot\n"
  printf "   [5] Alter. dominio UnicoBot\n"
  printf "\n"
  read -p "> " option
